...
function[face] = calculate_temporary_velocities(param,domain,fluid_prop,fluid,center,face)
    ...
    for i=2:domain.nx
      for j=2:domain.ny+1
        face.u_temp(i,j) = face.u(i,j)+param.dt*(-0.25* ...
            ...
            center.force_x(i,j)    /(0.5*(fluid.rho(i+1,j)+fluid.rho(i,j)))- ...
            (1.0 -fluid_prop(2).rho/(0.5*(fluid.rho(i+1,j)+fluid.rho(i,j)))) ...
            *domain.gravx);
      end
    end
    ...
    for i=2:domain.nx+1
      for j=2:domain.ny
        face.v_temp(i,j) = face.v(i,j)+param.dt*(-0.25* ...
            ...
            center.force_y(i,j)    /(0.5*(fluid.rho(i,j+1)+fluid.rho(i,j)))- ...
            (1.0 -fluid_prop(2).rho/(0.5*(fluid.rho(i,j+1)+fluid.rho(i,j)))) ...
            *domain.gravy);
      end
    end
